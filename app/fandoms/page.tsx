"use client";

import Link from "next/link";
import { useSearchParams } from "next/navigation";
import AuthStatus from "../../components/AuthStatus";

const fandoms = [
  {
    name: "House of the Dragon",
    description:
      "Histórias ambientadas no universo dos Targaryen e da Casa do Dragão.",
  },
  {
    name: "Game of Thrones",
    description:
      "Westeros, suas casas, personagens e histórias além da série.",
  },
  {
    name: "Harry Potter",
    description:
      "O mundo bruxo, Hogwarts e novas histórias de magia.",
  },
  {
    name: "Marvel",
    description:
      "Heróis, vilões, universos alternativos e novas aventuras.",
  },
  {
    name: "DC",
    description:
      "Gotham, Metropolis e os heróis e vilões da DC.",
  },
  {
    name: "K-pop",
    description:
      "Histórias inspiradas no universo dos artistas e grupos de K-pop.",
  },
];

const menu = [
  { label: "Início", href: "/" },
  { label: "Explorar", href: "/explorar" },
  { label: "Escrever", href: "/escrever" },
  { label: "Fandoms", href: "/fandoms" },
  { label: "Notícias", href: "/noticias" },
];

export default function FandomsPage() {
  const searchParams = useSearchParams();
  const selected = searchParams.get("fandom");

  return (
    <main className="min-h-screen bg-[#100b12] text-white">
      <header className="sticky top-0 z-50 border-b border-white/10 bg-[#100b12]/95 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center gap-6 px-5 py-4">
          <Link
            href="/"
            className="flex shrink-0 items-center gap-2"
          >
            <CloudLogo />

            <span className="text-xl font-black tracking-[0.18em] text-[#ff78b9]">
              NOOKLIE
            </span>
          </Link>

          <nav className="hidden flex-1 items-center justify-center gap-1 md:flex">
            {menu.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`rounded-full px-5 py-2.5 text-sm font-medium transition ${
                  item.href === "/fandoms"
                    ? "bg-gradient-to-r from-[#ff5fab] to-[#ff8fc5] text-[#180d15]"
                    : "text-white/60 hover:bg-[#ff78b9]/10 hover:text-[#ff9bca]"
                }`}
              >
                {item.label}
              </Link>
            ))}
          </nav>

          <div className="ml-auto">
            <AuthStatus />
          </div>
        </div>
      </header>

      <div className="mx-auto max-w-7xl px-5 py-12 md:py-16">
        <section className="relative overflow-hidden rounded-[2rem] border border-[#ff78b9]/15 bg-gradient-to-br from-[#291522] via-[#21131e] to-[#171018] p-8 md:p-12">
          <div className="pointer-events-none absolute -right-40 -top-40 h-96 w-96 rounded-full bg-[#ff78b9]/10 blur-[110px]" />

          <div className="relative">
            <p className="text-xs font-semibold uppercase tracking-[0.25em] text-[#ff78b9]">
              Comunidades
            </p>

            <h1 className="mt-3 text-4xl font-black md:text-5xl">
              Encontre seu fandom.
            </h1>

            <p className="mt-4 max-w-2xl leading-7 text-white/50">
              Explore histórias criadas por leitores e escritores que
              compartilham os mesmos universos que você.
            </p>
          </div>
        </section>

        {selected && (
          <div className="mt-8 rounded-2xl border border-[#ff78b9]/20 bg-[#ff78b9]/5 px-5 py-4">
            <p className="text-sm text-white/50">
              Fandom selecionado:
              <span className="ml-2 font-semibold text-[#ff9bca]">
                {selected}
              </span>
            </p>
          </div>
        )}

        <section className="mt-12">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {fandoms.map((fandom, index) => (
              <Link
                key={fandom.name}
                href={`/explorar?fandom=${encodeURIComponent(
                  fandom.name
                )}`}
                className="group relative overflow-hidden rounded-[1.5rem] border border-white/10 bg-[#171018] p-7 transition duration-300 hover:-translate-y-1 hover:border-[#ff78b9]/30 hover:bg-[#1c141b]"
              >
                <div
                  className={`absolute -right-16 -top-16 h-40 w-40 rounded-full bg-gradient-to-br ${
                    [
                      "from-[#ff78b9]/15 to-transparent",
                      "from-purple-400/15 to-transparent",
                      "from-blue-400/10 to-transparent",
                      "from-red-400/10 to-transparent",
                      "from-cyan-400/10 to-transparent",
                      "from-violet-400/10 to-transparent",
                    ][index]
                  } blur-2xl`}
                />

                <div className="relative">
                  <span className="text-xs font-semibold uppercase tracking-[0.18em] text-[#ff78b9]/60">
                    Fandom
                  </span>

                  <h2 className="mt-3 text-2xl font-black transition group-hover:text-[#ff9bca]">
                    {fandom.name}
                  </h2>

                  <p className="mt-3 text-sm leading-6 text-white/40">
                    {fandom.description}
                  </p>

                  <span className="mt-6 inline-block text-sm font-semibold text-[#ff78b9] transition group-hover:text-[#ff9bca]">
                    Explorar histórias →
                  </span>
                </div>
              </Link>
            ))}
          </div>
        </section>
      </div>

      <footer className="border-t border-white/10 bg-[#0b080d]">
        <div className="mx-auto max-w-7xl px-5 py-10 text-center">
          <p className="text-sm text-white/30">
            De escritor para escritor.
          </p>

          <p className="mt-2 text-xs text-white/15">
            © {new Date().getFullYear()} Nooklie
          </p>
        </div>
      </footer>
    </main>
  );
}

function CloudLogo() {
  return (
    <svg
      width="44"
      height="30"
      viewBox="0 0 180 105"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      aria-label="Logo Nooklie"
    >
      <path
        d="M45 79C26 79 14 67 14 51C14 36 25 24 40 23C45 9 58 2 73 2C89 2 102 12 106 27C111 24 117 22 124 22C143 22 158 36 158 54C158 57 158 60 157 63C168 66 174 74 174 84C174 96 164 103 151 103H45C28 103 17 94 17 83C17 81 17 80 18 78C26 79 35 79 45 79Z"
        fill="#FF78B9"
      />

      <path
        d="M45 79C26 79 14 67 14 51C14 36 25 24 40 23C45 9 58 2 73 2C89 2 102 12 106 27C111 24 117 22 124 22C143 22 158 36 158 54C158 57 158 60 157 63C168 66 174 74 174 84C174 96 164 103 151 103H45C28 103 17 94 17 83C17 81 17 80 18 78C26 79 35 79 45 79Z"
        stroke="#FF9BCB"
        strokeWidth="3"
      />
    </svg>
  );
}
